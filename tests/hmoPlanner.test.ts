...
function room(id: string, name: string, type: string, x: number, y: number, width: number, height: number): Room {
  return {
    ...
    approxAreaSqm: (width * height) / 10000,
    approxWidthM: width / 100,
    approxDepthM: height / 100,
    windows: [{ wall: "bottom" }],
    doors: [{ wall: "top" }],
  };
}
  ...
  const plan: FloorPlan = {
    floors: [
      { name: "Ground Floor", level: 0, rooms: [
        room("gf-lounge", "Lounge", "living", 0, 0, 340, 340),
        room("gf-living", "Living Room", "living", 340, 0, 440, 340),
        room("gf-kitchen", "Kitchen", "kitchen", 0, 340, 280, 300),
      ] },

...
type Event = {
  id: string; type: "MATCH" | "TRAINING"; title: string; opponent?: string | null; startsAt: string; arrivalTime?: string | null; venue: string; instructions?: string | null;
...
export default function HomePolls() {
  ...
  useEffect(() => { fetch("/api/coachhub/public", { cache: "no-store" }).then((res) => res.json()).then((data: Data) => setEvents((data.events || []).slice(0, 4))).catch(() => setEvents([])).finally(() => setLoading(false)); }, []);
  ...
  return <section className="mt-10 space-y-4 text-left">
    ...
    {events.length === 0 ? <div className="rounded-3xl border border-slate-800 bg-slate-900 p-6 text-slate-400">No upcoming polls have been published yet.</div> : <div className="grid gap-4 md:grid-cols-2">{events.map((event) => <article key={event.id} className="rounded-3xl border border-slate-800 bg-slate-900 p-5 shadow-xl">
      <p className="text-xs font-black uppercase tracking-widest text-blue-400">{event.type === "MATCH" ? "⚽ Match poll" : "🏃 Training poll"}</p><h3 className="mt-2 text-xl font-black">{event.title}{event.opponent ? ` vs ${event.opponent}` : ""}</h3><p className="mt-2 text-sm text-slate-400">{fmt(event.startsAt)} · {event.venue}</p>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs font-bold"><div className="rounded-xl bg-emerald-950 p-3 text-emerald-300"><b className="block text-lg">{event.counts.available}</b>Available</div><div className="rounded-xl bg-rose-950 p-3 text-rose-300"><b className="block text-lg">{event.counts.unavailable}</b>Not available</div><div className="rounded-xl bg-slate-950 p-3 text-slate-400"><b className="block text-lg">{event.counts.pending}</b>Awaiting</div></div>

...
function resolveRoom(plan: any, label: RoomLabel): any | undefined { const rooms = plan.floors.flatMap((f: any) => f.rooms); const requested = String(label.roomId ?? "").trim().toLowerCase(); if (requested) { const exact = rooms.find((r: any) => String(r.id).trim().toLowerCase() === requested); if (exact) return exact; } const number = roomIdNumber(label.roomId) ?? roomIdNumber(label.name); if (number === undefined) return undefined; const candidates = rooms.filter((r: any) => roomIdNumber(r.id) === number); return candidates.length === 1 ? candidates[0] : undefined; }
function applyOneLabel(room: any, label: RoomLabel): void { if (label.name) room.name = String(label.name); if (label.type) room.type = String(label.type); if (label.confidence) room.confidence = String(label.confidence); if (Number(label.areaSqm) > 0) room.approxAreaSqm = Number(label.areaSqm); if (Number(label.widthM) > 0) room.approxWidthM = Number(label.widthM); if (Number(label.depthM) > 0) room.approxDepthM = Number(label.depthM); if (Array.isArray(label.windows) && label.windows.length) room.windows = label.windows.filter((w): w is WallSide => WALLS.includes(w)); if (Array.isArray(label.doors) && label.doors.length) room.doors = label.doors.filter((w): w is WallSide => WALLS.includes(w)).map(wall => ({ wall })); }
...
async function classifyRoomsAgain(original: any, image: string): Promise<RoomLabel[]> {
  ...
  const response = await openai.responses.create({ model: "gpt-5-mini", text: { format: { type: "json_object" } }, input: [{ role: "user", content: [

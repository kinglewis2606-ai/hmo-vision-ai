...
export async function POST(request: Request) {
  ...
  if (body.action === "availability") {
    ...
    if (!teamId) return NextResponse.json({ error: "Team access required" }, { status: 403 });
    ...
    const event = await prisma.event.findFirst({ where: { id: String(eventId), teamId } });
    const player = await prisma.player.findFirst({ where: { id: String(playerId), teamId } });
    if (!event || !player) return NextResponse.json({ error: "Event or player not found" }, { status: 404 });
    return NextResponse.json(await prisma.availability.upsert({ where: { eventId_playerId: { eventId: String(eventId), playerId: String(playerId) } }, update: { status, reason: null, respondedAt: new Date() }, create: { eventId: String(eventId), playerId: String(playerId), status, respondedAt: new Date() } }));
  }
  ...
  if (body.action === "event") {
    ...
    const title = String(body.title || "").trim(); const venue = String(body.venue || "").trim(); const instructions = String(body.instructions || "").trim() || null; const startsAt = new Date(String(body.startsAt || ""));
    ...
    const endsAt = body.endsAt ? new Date(String(body.endsAt)) : null; const arrivalTime = body.arrivalTime ? new Date(String(body.arrivalTime)) : null;
  ...
  if (body.action === "message") {
    const message = String(body.message || "").trim(); if (!message) return NextResponse.json({ error: "Message is required" }, { status: 400 });
  ...
  if (body.action === "squad") {
    const eventId = String(body.eventId || ""); const playerId = String(body.playerId || ""); const role = body.role; const position = body.position ? String(body.position).trim() : null; const isCaptain = Boolean(body.isCaptain);
    if (!eventId || !playerId || !squadRoles.includes(role)) return NextResponse.json({ error: "Invalid squad selection" }, { status: 400 });
    const event = await prisma.event.findFirst({ where: { id: eventId, teamId: team.id }); const player = await prisma.player.findFirst({ where: { id: playerId, teamId: team.id } });
    ...
    const selection = await prisma.$transaction(async (tx) => { if (isCaptain) await tx.matchSelection.updateMany({ where: { eventId, isCaptain: true }, data: { isCaptain: false } }); return tx.matchSelection.upsert({ where: { eventId_playerId: { eventId, playerId } }, update: { role, position, isCaptain }, create: { eventId, playerId, role, position, isCaptain } }); });
    return NextResponse.json(selection);
  }
  if (body.action === "match") {
    const eventId = String(body.eventId || ""); const event = await prisma.event.findFirst({ where: { id: eventId, teamId: team.id, type: "MATCH" } }); if (!event) return NextResponse.json({ error: "Match not found" }, { status: 404 });
    const scoreFor = body.scoreFor === "" || body.scoreFor == null ? null : Number(body.scoreFor); const scoreAgainst = body.scoreAgainst === "" || body.scoreAgainst == null ? null : Number(body.scoreAgainst);
  ...
  if (body.action === "feedback") {
    ...
    const linkedEvent = eventId ? await prisma.event.findFirst({ where: { id: String(eventId), teamId: team.id } }) : null;
    if (!player || (eventId && !linkedEvent) || (!wentWell && !needsWork && !focusNext)) return NextResponse.json({ error: "Player, valid event and feedback are required" }, { status: 400 });
    return NextResponse.json(await prisma.feedback.create({ data: { playerId: player.id, eventId: linkedEvent?.id || null, coachName: user.email, wentWell, needsWork, focusNext, passing, firstTouch, positioning, confidence } }));
  }

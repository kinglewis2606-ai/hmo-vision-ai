...
export async function POST(request: Request) {
  ...
  if (body.action === "availability") {
    ...
    if (user.role === "PLAYER" && user.playerId !== player.id) return NextResponse.json({ error: "You can only answer your own poll" }, { status: 403 });
    if (user.role === "PARENT") {
      const approved = await prisma.parentAccessRequest.findFirst({ where: { userId: user.id, teamId: team.id, playerId: player.id, status: "APPROVED" } });
      if (!approved) return NextResponse.json({ error: "Coach approval is required for this child" }, { status: 403 });
    }
    if (user.role !== "COACH" && user.role !== "PLAYER" && user.role !== "PARENT") return NextResponse.json({ error: "Role cannot answer polls" }, { status: 403 });
  ...
  if (user.role !== "COACH") return NextResponse.json({ error: "Coach access required" }, { status: 403 });
  ...
  if (body.action === "match") {
    const eventId = String(body.eventId || ""); const event = await prisma.event.findFirst({ where: { id: eventId, teamId: team.id, type: "MATCH" } });
  ...
  if (body.action === "feedback") {
    ...
    return NextResponse.json(await prisma.feedback.create({ data: { playerId: player.id, eventId: eventId || null, coachName: user.email, wentWell, needsWork, focusNext, passing, firstTouch, positioning, confidence } }));
  }

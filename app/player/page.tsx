...
export default function PlayerPage() {
  ...
  const setAvailability = async (event: Event, status: "AVAILABLE" | "UNAVAILABLE") => {
    if (!player || saving) return;
    setSaving(event.id + status); setMessage("");
    const res = await fetch("/api/coachhub", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "availability", teamId: "", eventId: event.id, playerId: player.id, status }) });
    if (res.ok) { await load(); setMessage("Availability updated."); } else setMessage((await res.json()).error || "Could not update availability.");
    setSaving("");
  };

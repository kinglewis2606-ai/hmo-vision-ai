...
function usableFloor(f:{name?:string;x?:number;y?:number;width?:number;height?:number},w:number,h:number){const x=Math.max(0,Math.round(Number(f.x??0))),y=Math.max(0,Math.round(Number(f.y??0)));const right=Math.min(w,Math.round(x+Number(f.width??0))),bottom=Math.min(h,Math.round(y+Number(f.height??0)));if(right-x<40||bottom-y<40)return undefined;return{name:String(f.name||"Floor"),x,y,width:right-x,height:bottom-y};}
...
async function detectPlan(filePath:string,width:number,height:number,context:DetectionContext):Promise<VisionPlan>{
  const source=fs.readFileSync(filePath); const scale=Math.min(1,1800/Math.max(width,height)); const iw=Math.max(1,Math.round(width*scale)); const ih=Math.max(1,Math.round(height*scale));
...
Coordinates are pixels in the supplied ${iw}x${ih} image.`;
  const response=await openai.responses.create({model:"gpt-5-mini",input:[{role:"user",content:[{type:"input_text",text:prompt},{type:"input_image",image_url:`data:image/jpeg;base64,${image.toString("base64")}`,detail:"high"}]}],max_output_tokens:5000});
...
export async function detectFloors(filePath:string,context:DetectionContext={}):Promise<DetectedFloor[]>{
  const metadata=await sharp(filePath).metadata(); const width=metadata.width??0,height=metadata.height??0;if(!width||!height)return[];const stat=fs.statSync(filePath);const key=`${filePath}:${stat.size}:${stat.mtimeMs}:${context.address||""}:${context.propertyType||""}`;if(cacheKey===key&&cache)return floorResult(cache,width,height);
  const raw=await detectPlan(filePath,width,height,context);const scale=Math.min(1,1800/Math.max(width,height));let floors=(raw.floors||[]).map(f=>({...f,x:Number(f.x||0)/scale,y:Number(f.y||0)/scale,width:Number(f.width||0)/scale,height:Number(f.height||0)/scale})).map(f=>usableFloor(f,width,height)).filter((f):f is NonNullable<typeof f>=>!!f);if(!floors.length)floors=[{name:"Ground Floor",x:0,y:0,width,height}];
  const rooms=dedupeRooms((raw.rooms||[]).map(r=>({...r,floorIndex:Number.isFinite(Number(r.floorIndex))?Number(r.floorIndex):0,x:Number(r.x)/scale,y:Number(r.y)/scale,width:Number(r.width)/scale,height:Number(r.height)/scale,polygon:validPolygon({...r,x:Number(r.x)/scale,y:Number(r.y)/scale,width:Number(r.width)/scale,height:Number(r.height)/scale})})).filter(r=>validRoom(r,width,height)&&!!r.polygon));
  cacheKey=key;cache={...raw,floors,rooms};console.log(`Vision detection complete: ${floors.length} floor(s), ${rooms.length} room(s), ${raw.changes?.length||0} strategy change(s) in one AI pass`);return floorResult(cache,width,height);
}

...
function normalisePolygon(room: VisionRoom, width: number, height: number): Point[] | undefined {
  ...
  if (polygon.some(p => !Number.isFinite(p.x) || !Number.isFinite(p.y) || p.x < 0 || p.y < 0 || p.x > width || p.y > height)) return undefined;
  const xs = polygon.map(p => p.x), ys = polygon.map(p => p.y);
  if (Math.max(...xs) - Math.min(...xs) < 20 || Math.max(...ys) - Math.min(...ys) < 20) return undefined;
  return polygon;
}
...
async function detectWithVision(filePath: string): Promise<VisionPlan | null> {
  ...
  const response = await openai.responses.create({
    ...
    input: [{ role: "user", content: [
      { type: "input_text", text: `Act as a strict architectural floor-plan geometry detector. Detect the actual floor-plan panels and every distinct enclosed room in this image. Do not assume three floors, equal-sized panels, a fixed orientation, or a fixed room count. Return JSON only: {"floors":[{"name":"","x":0,"y":0,"width":0,"height":0}],"rooms":[{"floorIndex":0,"x":0,"y":0,"width":0,"height":0,"polygon":[{"x":0,"y":0},{"x":0,"y":0},{"x":0,"y":0},{"x":0,"y":0}]}]}. Coordinates MUST be pixels in the original ${width}x${height} image, scaled back from the supplied image if necessary. A room boundary is the interior face of its enclosing walls; do not draw a box around printed text, furniture, whitespace, a compass, a watermark, or a whole floor panel. Each room box/polygon MUST sit directly over a genuinely enclosed room visible in the source image. Follow wall lines and door openings. Do not invent rooms. Do not split one room because of furniture or labels. Prefer a slightly conservative boundary inside the walls over a floating or oversized rectangle. Include every enclosed room: bedrooms, living/lounge, dining, kitchen, bathroom, shower room, WC, hall, landing, stairs, utility/storage and other distinct spaces.` },
...
async function verifyAndCorrectRooms(filePath: string, detected: VisionPlan, width: number, height: number): Promise<VisionPlan> {
  ...
  const source = fs.readFileSync(filePath);
  const image = await sharp(source).resize({ width: 1800, height: 1800, fit: "inside", withoutEnlargement: true }).jpeg({ quality: 86, mozjpeg: true }).toBuffer();
  ...
  try {
    const response = await openai.responses.create({
      model: "gpt-5-mini",
      input: [{ role: "user", content: [
        { type: "input_text", text: `You are the final geometry verification pass for a floor-plan analysis. The supplied image is the ORIGINAL floor plan. Candidate room rectangles are provided below. Inspect the actual wall lines in the image and correct each candidate so its rectangle/polygon corresponds to the real enclosed room, not blank page space, text, a watermark, compass, or a neighbouring room. Preserve a candidate only if it is a real enclosed room. You may adjust x/y/width/height and return an optional polygon following the visible room boundary. Do not invent additional rooms and do not move a real room to another floor. Coordinates are in the ORIGINAL ${width}x${height} image. Return JSON only: {"rooms":[{"candidateId":1,"floorIndex":0,"x":0,"y":0,"width":0,"height":0,"polygon":[{"x":0,"y":0},{"x":0,"y":0},{"x":0,"y":0},{"x":0,"y":0}]}]}. A candidate is valid only when it visibly overlays an enclosed room in the source image. Reject floating squares or rectangles that do not coincide with walls. Candidates: ${JSON.stringify(candidates)}` },
        { type: "input_image", image_url: `data:image/jpeg;base64,${image.toString("base64")}`, detail: "high" },
      ] }],
    });
    const parsed = JSON.parse(cleanJson(response.output_text || "{}"));
    if (!Array.isArray(parsed.rooms)) return detected;
    const corrected: VisionRoom[] = parsed.rooms.map((r: any) => ({ floorIndex: Number.isFinite(Number(r.floorIndex)) ? Number(r.floorIndex) : undefined, x: Number(r.x), y: Number(r.y), width: Number(r.width), height: Number(r.height), polygon: r.polygon })).filter((r: VisionRoom) => validRoom(r, width, height));
    if (corrected.length < Math.max(2, Math.floor(rooms.length * 0.55))) return detected;
    return { ...detected, rooms: corrected };
  } catch (error) {
    console.warn("Room geometry verification failed; retaining first-pass geometry", error);
    return detected;
  }
}
...
export async function detectFloors(filePath: string): Promise<DetectedFloor[]> {
  ...
  if (cachePath === filePath && cache?.floors?.length) {
    return cache.floors.map((floor, index) => ({ name: floor.name || `Floor ${index + 1}`, level: index, top: Math.max(0, Math.round(floor.y || 0)), left: Math.max(0, Math.round(floor.x || 0)), bottom: Math.min(height, Math.round((floor.y || 0) + (floor.height || 0))), right: Math.min(width, Math.round((floor.x || 0) + (floor.width || 0))) }));
  }
  try {
...
export function getVisionDetectedRooms(filePath: string): DetectedRoom[] | null {
  ...
  return cache.rooms.map((room, index) => {
    const x = Math.round(Number(room.x) || 0), y = Math.round(Number(room.y) || 0), width = Math.round(Number(room.width) || 0), height = Math.round(Number(room.height) || 0);
    const polygon = normalisePolygon(room, Math.max(width, x + width), Math.max(height, y + height));

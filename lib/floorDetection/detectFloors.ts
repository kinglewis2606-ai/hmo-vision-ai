...
async function detectFloorPanels(filePath: string, width: number, height: number): Promise<VisionFloor[]> {
  ...
  try {
    const r = await openai.responses.create({ model: "gpt-5-mini", text: { format: { type: "json_object" } }, input: [{ role: "user", content: [
  ...
  } catch (e) { console.warn("Floor panel detection failed; using full-image fallback", e); return []; }
}
...
async function detectRoomsPerFloor(filePath: string, width: number, height: number, floor: VisionFloor, floorIndex: number): Promise<VisionRoom[]> {
  ...
  try {
    ...
    return parsed.rooms.map((q: any) => ({ floorIndex, x: Number(q.x) * sx + left, y: Number(q.y) * sy + top, width: Number(q.width) * sx, height: Number(q.height) * sy, polygon: Array.isArray(q.polygon) ? q.polygon.map((v: any) => ({ x: Number(v.x) * sx + left, y: Number(v.y) * sy + top })) : undefined })).filter((q: VisionRoom) => validRoom(q, width, height) && !!validPolygon(q));
  } catch (e) { console.warn(`Room detection failed on floor ${floorIndex + 1}`, e); return []; }
}
...
async function verifyRooms(filePath: string, plan: VisionPlan, width: number, height: number): Promise<VisionPlan> {
  ...
  try {
    ...
    if (acceptedIds.size < minimumAccepted) { console.warn(`Room verification rejected too many candidates (${acceptedIds.size}/${rooms.length}); retaining primary geometry`); return { ...plan, rooms }; }
    return { ...plan, rooms: rooms.filter((_, index) => acceptedIds.has(index + 1)) };
  } catch (e) { console.warn("Room geometry verification failed; retaining primary geometry", e); return { ...plan, rooms }; }
}
...
export async function detectFloors(filePath: string): Promise<DetectedFloor[]> {
  ...
  try {
    ...
    if (!floors.length) { console.warn("No floor panel detected; using full-image architectural panel fallback"); floors = [{ name: "Ground Floor", x: 0, y: 0, width, height }]; }
    ...
    cacheKey = currentKey; cache = plan;
  ...
  } catch (e) { console.warn("Vision floor/room detection failed", e); cacheKey = currentKey; cache = { floors: [], rooms: [] }; return []; }
}
...
export function getVisionDetectedRooms(filePath: string): DetectedRoom[] | null {
  ...
  return (cache.rooms || []).map((r, i) => { const x = Math.round(Number(r.x) || 0), y = Math.round(Number(r.y) || 0), width = Math.round(Number(r.width) || 0), height = Math.round(Number(r.height) || 0), polygon = validPolygon(r); return { id: `room-${i + 1}`, x, y, width, height, ...(polygon ? { polygon } : {}) }; }).filter(r => r.width >= 20 && r.height >= 20);
}

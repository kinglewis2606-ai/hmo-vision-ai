...
function openingIntervals(room: Room, kind: "door" | "window") {
  ...
  return openings.map(opening => {
    ...
    const startWall = horizontal ? room.x : room.y;
    const endWall = horizontal ? room.x + room.width : room.y + room.height;
    ...
    const span = endWall - startWall;
    // Wall-only opening data is incomplete. Reserve a conservative central interval
    // rather than treating the entire wall as unusable.
    const reserve = Math.min(span * 0.30, 120), centre = (startWall + endWall) / 2;
...
function principalWindowWalls(room: Room): Set<WallSide> {
  ...
  const longest = Math.max(...walls.map(length));
  return new Set(walls.filter(w => length(w) >= longest * 0.75));
}

function openingWallBlocked(room: Room, cut: Bounds): boolean {
  const edges: Array<[WallSide, boolean]> = [
    ["top", Math.abs(cut.y - room.y) <= 2],
    ["bottom", Math.abs(cut.y + cut.height - (room.y + room.height)) <= 2],
    ["left", Math.abs(cut.x - room.x) <= 2],
    ["right", Math.abs(cut.x + cut.width - (room.x + room.width)) <= 2],
  ];
  return edges.some(([wall, at]) => at && openingIntervals(room, "door").some(o => {
    if (o.wall !== wall) return false;
    const start = wall === "top" || wall === "bottom" ? cut.x : cut.y;
    const end = wall === "top" || wall === "bottom" ? cut.x + cut.width : cut.y + cut.height;
...
function preserveOpenings(room: Room, remainder: Point[]) {
  const survives = (wall: WallSide, start?: number, end?: number) => {
    ...
    return remainder.some((p, i) => {
      ...
      const a = horizontal ? Math.min(p.x, q.x) : Math.min(p.y, q.y);
      const b = horizontal ? Math.max(p.x, q.x) : Math.max(p.y, q.y);
      return Math.max(a, min) < Math.min(b, max);
    });
  };
  return {
    windows: (room.windows || []).filter(w => survives(w.wall, w.start, w.end)),
    doors: (room.doors || []).filter(d => survives(d.wall, d.start, d.end)),
  };
}
...
function splitRoomGeometry(floor: { rooms: Room[] }, room: Room, change: RoomChange): boolean {
  ...
  const first = clip(original.polygon, direction === "horizontal" ? "y" : "x", threshold, false);
  const second = clip(original.polygon, direction === "horizontal" ? "y" : "x", threshold, true);
  ...
  if (!validateBedroomGeometry(firstRoom).valid || !validateBedroomGeometry(secondRoom).valid) return false;
  if (!areasConserve(original, first, second)) return false;
  ...
  child.windows = secondOpen.windows; child.doors = secondOpen.doors; child.approxAreaSqm = Number(sqmForPolygon(original, second).toFixed(2));
...
function findEnsuite(room: Room): { polygon: Point[]; remainder: Point[]; areaSqm: number } | undefined {
  ...
  const metresPerPixel = sourcePx > 0 && sourceSqm > 0 ? Math.sqrt(sourceSqm / sourcePx) : 0;
  for (const corner of candidateCorners(room)) for (const wf of [0.22, 0.26, 0.30, 0.34, 0.38]) for (const hf of [0.22, 0.26, 0.30, 0.34, 0.38]) {
    const cut = buildCornerRect(room, corner, room.width * wf, room.height * hf);
    if (openingWallBlocked(room, cut)) continue;
    ...
    if (!remainder || !areasConserve(room, remainder, polygon)) continue;
    const openings = preserveOpenings(room, remainder), principal = principalWindowWalls(room);
    if (!openings.doors?.length) continue;
    if (principal.size && !Array.from(principal).every(w => (openings.windows || []).some(x => x.wall === w))) continue;
    const bedroomRemainder = { ...structuredClone(room), polygon: remainder, windows: openings.windows, doors: openings.doors, type: "bedroom" };
    if (!validateBedroomGeometry(bedroomRemainder).valid) continue;
    const ensuiteArea = sqmForPolygon(room, polygon);
    if (ensuiteArea < ENSUITE_TARGET_SQM * 0.75) continue;
    const widthM = room.approxWidthM && room.width ? cut.width * room.approxWidthM / room.width : cut.width * metresPerPixel;
    const heightM = room.approxDepthM && room.height ? cut.height * room.approxDepthM / room.height : cut.height * metresPerPixel;
    if (widthM < ENSUITE_SHOWER_MIN_M || heightM < ENSUITE_SHOWER_MIN_M) continue;
...
function addEnsuiteGeometry(floor: { rooms: Room[] }, room: Room, change: RoomChange): boolean {
  const candidate = findEnsuite(room);
  ...
  Object.assign(room, remainder, { approxAreaSqm: Number(checked.areaSqm.toFixed(2)), confidence: "geometry-proposed" });
  const child = childRoom(original, candidate.polygon, "ensuite", "En-suite", `Real geometry carved from ${original.id}`);
  child.approxAreaSqm = Number(candidate.areaSqm.toFixed(2));
  floor.rooms.push(child);
  return true;
}
...
function applySimple(room: Room, change: RoomChange): boolean {
  ...
  if (!requested || noOp(room.type || "", requested)) return false;
  room.type = change.newType || actionType(change.action) || room.type;
  if (change.newName) room.name = change.newName; else if (requested === "bedroom" && !/bedroom/i.test(room.name)) room.name = "Proposed Bedroom";

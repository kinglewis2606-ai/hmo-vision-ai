...
function roomFill(room: any, floorIndex: number): string {
  if (isEnsuite(room)) return "#38bdf8";
  const fills = ["#facc15", "#86efac", "#f0a3d7", "#c4b5fd", "#fdba74"];
  return fills[Math.max(0, floorIndex) % fills.length];
}
function renderRoom(room: any, label: string, kind: "bedroom" | "ensuite", floorIndex: number, clipId?: string): string {
  ...
  const shape = points
    ? `<polygon points="${points}" fill="${fill}" fill-opacity="0.58" stroke="${stroke}" stroke-width="5" vector-effect="non-scaling-stroke"/>`
    : `<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${fill}" fill-opacity="0.58" stroke="${stroke}" stroke-width="5"/>`;
  const center = roomCenter(room);
  const font = Math.max(10, Math.min(20, Math.min(w, h) / 5));
  const badge = Math.max(70, Math.min(145, label.length * 10 + 28));
  const badgeH = Math.max(22, Math.min(38, h * 0.18));
  const bx = center.x - badge / 2, by = center.y - badgeH / 2;
...
export function renderFloorPlan(original: FloorPlan, proposed: FloorPlan, originalImageDataUri: string, changes: RoomChange[] = []): string {
  ...
  // Hide only the source geometry that is actually being changed. The
  // untouched plan remains visible underneath, so the proposed image always
  // stays recognisable as the uploaded property.
  ...
  // Bedrooms are numbered from the final applied geometry, never from the AI
  // proposal. This prevents gaps such as Bedroom 7 when a candidate was rejected.
  proposedBedrooms.forEach(({ room, floorIndex, id }, index) => {
    ...
    overlays.push(renderRoom(room, label, "bedroom", floorIndex, clipId));
    rendered.add(id);
  });

  // En-suites are rendered from their actual child polygons. They are not
  // painted over the bedroom: applyRoomChanges is responsible for subtracting
  // them, and this renderer simply visualises the resulting geometry.
  proposedEnsuites.forEach(({ room, floorIndex }) => {
    overlays.push(renderRoom(room, "En-suite", "ensuite", floorIndex));
  });

  // Render other explicit room conversions after bedrooms/ensuites.
  for (const change of changes) {
    ...
    overlays.push(renderRoom(room, String(room.name || room.type || "PROPOSED").toUpperCase(), "bedroom", floor?.level ?? 0, clipId));
  }
